...
#df_cash_flow_list = pd.DataFrame(columns=["Iteration","Day","Total accrued cost"]) 
...
for i in total_iterations:
    ...
    for s in duration:
        ...
        #calculate total cost per duration cycle and create th cash flow list
        #for itr in classes.iteration._registry:
        #    itr.total_cost = sum(b.accrued_cost for b in classes.block._registry)
        #df_cash_flow_list.loc[len(df_cash_flow_list.index)] = [i,s,itr.total_cost]
        
        #print (s, itr.total_cost)


    for block in classes.block._registry:
        if block.accrued_cost > 0:
...
#fileIO.df_to_csv(df_cash_flow_list,"Cash_Flow_List.csv")