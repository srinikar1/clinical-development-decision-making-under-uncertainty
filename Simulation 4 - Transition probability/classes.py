...
class block:
    ...
    def __update_block__ (self, current_day):
        if self.status == "Starting":
            ...
            # Create an activity and insert it into the block queue
            #activity = classes.activity(1, "Activity", "running", current_day, 0)
            #activity.name = self.name
            #activity.duration = self.duration
            #self.activities.append(activity)
            # print (activity.name, "xx")
    ...
    def __end_block__ (self, current_day):
        ...
        if self.status == "Running" and self.worked_days == self.duration:
            ...
            # print (self.name, self.end_day, current_day, "is complete")
...
class iteration:
    ...
    def __init__ (self, id, iteration, total_cost):
        self._registry.append(self) #Add the newly created object to the register
        self.id = id
